...
export const EC3Widget = (): any => {
  ...
  const [carbonAPI, setCarbonAPI] = useState<any>();
  const [carbonAPILoggedIn, setCarbonAPILoggedIn] = React.useState(false);
  const [ec3LoggedIn, setEC3LoggedIn] = useState(false);
  const ec3API = React.useRef<any>({});
  const [ec3UserName, setEC3UserName] = useState("");
  const [ec3Password, setEC3Password] = useState("");
  ...
  useEffect(() => {
    ec3API.current = new EC3Api(setCarbonAPILoggedIn);
    setCarbonAPI(ec3API);
    return () => {
      setElements([]);
      setColumns([]);
  ...
  useEffect(() => {
    if (iModelId && claims.email && accessToken && carbonAPILoggedIn) {
      void ec3API.current
  ...
  }, [iModelId, accessToken, claims, carbonAPILoggedIn, carbonAPI]);

  useEffect(() => {
    if (iModelId && carbonAPILoggedIn) {
      if (EPDMappingLoaded) {
        const materialList: string[] = [];
        for (const aMapping of epdMapping.groups) {
          materialList.push(aMapping.material);
        }
        void ec3API.current.getAllEPD(materialList).then((allEPD: any) => {
          setEPD(allEPD);
          setEPDLoaded(true);
        });
      }
    }
  }, [iModelId, carbonAPILoggedIn, EPDMappingLoaded, epdMapping]);
  ...
  useEffect(() => {
    ...
    try {
      if (!elementsLoaded && !isLoading.current && groupsLoaded && epdLoaded) {
        isLoading.current = true;
  ...
  const EC3Login = async () => {
    ec3API.current.login(ec3UserName, ec3Password);
  };
      ...
      <div>
        <div className="ec3-signin">
          ...
          <LabeledInput
            ...
            displayStyle="inline"
            onChange={(v) => setEC3UserName(v.target.value)}
            width="250px"
          />
          ...
          <LabeledInput
            ...
            displayStyle="inline"
            onChange={(v) => setEC3Password(v.target.value)}
            width="250px"
