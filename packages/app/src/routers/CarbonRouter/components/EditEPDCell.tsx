...
export const EditEPDCell = ({ children, ...props }: any) => {
  const { row, epdOptions, mapping, storeMapping } = props;

  const [optionMaterial, setOptionMaterial] = React.useState(
  ...
  return (
    <ComboBox
      value={optionMaterial}
      options={epdOptions}
      onChange={(value: string) => saveCategoryEPD(value)}
